            ...
            {availableCount > 0 && firstAvailableDate && (
                ...
                >
                    <div>
                        ...
                        {availableCount === 1 ? "entry" : "entries"} with available time slot{availableCount === 1 ? "" : "s"}. You
                                ...
                                <div className="*:not-first:mt-2">
                                    ...
                                    <Popover>
                                        <PopoverTrigger asChild>
                                            ...
                                            >
                                                <span className={cn("truncate", !row.date && "text-muted-foreground")}>
                                    ...
                                    <Popover>
                                        ...
                                        <PopoverContent className="w-auto p-0" align="start">
                                            <TimeRangePicker
                                                ...
                                                onDone={(start, end) => {
                                                    if (!start && !end) {
                                                        onChange(idx, { time: "Any time", startTime: "", endTime: "" })
                                                    } else {
                                                        const displayTime = `${start ?? "12:00"} - ${end ?? "17:00"}`
                                                        onChange(idx, { time: displayTime, startTime: start ?? "", endTime: end ?? "" })
                                                    }
                                                }}
                                                onClear={() => onChange(idx, { time: "Any time", startTime: "", endTime: "" })}
                                            />
                                        </PopoverContent>
                                    </Popover>
                                </div>
